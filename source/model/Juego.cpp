...
vector<tuple<string,SDL_Rect, SDL_Rect ,SDL_RendererFlip >>Juego::dibujar(){
	...
	if(p1->arrojando()) dibujables.push_back(make_tuple(p1->getNombre(),p1->getRectOrigenArrojable(),p1->getRectDestinoArrojable(),p1->getFlip()));
	if(p2->arrojando()) dibujables.push_back(make_tuple(p2->getNombre(),p2->getRectOrigenArrojable(),p2->getRectDestinoArrojable(),p2->getFlip()));
	p1->actualizar(p2);
	p2->actualizar(p1);
