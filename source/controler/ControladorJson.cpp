...
void ControladorJson::setPantallaCompleta(json j)throw(){
	try{
		...
		controladorLogger->registrarEvento("ERROR","Valor no permitido para fullscreen.");
...
void ControladorJson::setCantidadPersonajes(json j)throw(){
	try{
		...
		controladorLogger->registrarEvento("ERROR","Cantidad de personajes invalida. Se setea en: " + std::to_string(cantidad_personajes_default));
...
void ControladorJson::setCantidadFondos(json j)throw(){
	try{
		...
		controladorLogger->registrarEvento("ERROR","Cantidad de fondos invalida. Se setea en: " + std::to_string(cantidad_fondos_default));
	}
...
void ControladorJson::setPersonajes(json j)throw(){
	for (int i = 0; i < cantidad_personajes; i++){
		...
		if (file.good() == false){
			//Cargar imagen con zindex correspondiente
			controladorLogger->registrarEvento("ERROR","Imagen de personaje no encontrada. Se carga una por defecto");
